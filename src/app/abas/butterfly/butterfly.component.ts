...
export class ButterflyComponent implements OnInit {
  ...
  private filtros() {
    ...
    console.log('Dados Filtrados Butterfly: ', this.dados_aux);
  }
  ...
  filtrarPrecoAtual(preco: Butterfly[]) {
    preco.filter(p => {
  ...
  filtrarPrecosSaida(preco: Butterfly[]) {
    preco.filter(pre => {
      this.precosSaida.push(pre.preco_saida_atual);
    });
  }

  filtrarParametroMontagem(pamMontagem: Butterfly[]) {
